...
def scan_ftp_directory(
    ...
    try:
        ...
        if files_metadata:
            last_obs_dt = None
            for _, meta in enumerate(files_metadata):
...
def print_results(
    ...
    for i, meta in enumerate(files_metadata, 1):
        ...
        print(
            f"{i:<4} " f"{meta.filename:<50} " f"{obs_str:<20} " f"{arr_str:<20} " f"{delay_str:<10} " f"{status:<10}"
        )
    ...
    if delays:
        ...
        if max_delay - min_delay > 300:  # More than 5 minutes variance
            ...
            print("     This could indicate files arriving out of order or FTP timing issues.")

        if out_of_order_count > 0:
            print(f"\n  ✗ ALERT: {out_of_order_count} file(s) arrived out of chronological order!")
            print("     Files with older observation times arrived AFTER files with newer times.")
